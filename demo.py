...
from Models import compile_contrastive_model, compile_triplet_model, compile_facenet_model
...
def train_contrastive_model(X, train_triplets, test_triplets, batch_size=64, epochs=10, plot=False):
    """
        Builds and trains the contrastive (Siamese) model.
        Returns the trained embedding model and the contrastive network.
    """
    contrast_net, embedding_model = compile_contrastive_model(X.shape[1:])
    # Train model
    print("Training model")
    history = contrast_net.fit(
    ...
    if plot:
        plot_training_history(history, "Contrastive")
    return embedding_model, contrast_net

def train_triplet_model(X, train_triplets, test_triplets, batch_size=64, epochs=10, plot=False):
    triplet_net, embedding_model = compile_triplet_model(X.shape[1:])
    # Train model
    print("Training model")
...
def train_facenet_model(X, train_triplets, test_triplets, batch_size=64, epochs=10, plot=False):
    triplet_net, embedding_model = compile_facenet_model(X.shape[1:])
    # Train model
    print("Training model")
    history = triplet_net.fit(
        triplet_generator(X, train_triplets, batch_size),
    ...
    if plot:
        plot_training_history(history, "FaceNet")
    return embedding_model, triplet_net

def evaluate_model(embedding_model, test_triplets, X, y, target_names,plot=False):
    """
        Evaluates the trained embedding model and plots results.
    """
    print("Evaluating model")
    pairs, pair_labels = pairs_from_triplets(test_triplets)
    threshold = evaluate_verification(embedding_model, pairs, pair_labels, X,verbose=True,plot=plot)
...
if __name__ == "__main__":
    ...
    split = 0.2
    # Load dataset
    X_train, y_train, X_test, y_test, target_names = get_data_person(min_pics=20, color=True, size=(160, 160), split=split)
