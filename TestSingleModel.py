...
def main():
    ...
    X_train, y_train, X_test, y_test, target_names = get_data_image(size=(160,160),min_pics=2)
    X_train, y_train = augmentor.augment_batch(X_train, y_train)
    X_train = np.array(X_train, dtype=np.float32)/255.0 *2 -1
    X_test = np.array(X_test, dtype=np.float32)/255.0 *2 -1
    # Create triplets (anchor, positive, negative)

    print("Generating triplets")
    train_triplets = create_triplets(y_train, num_triplets=4000)
    ...
    full_network, embedding_model = compile_facenet_model(X_train.shape[1:])
    ...
    threshold = evaluate_verification(embedding_model, pairs, pair_labels, X_test)
    pairs, pair_labels = pairs_from_triplets(test_triplets, n_pairs=8, other=True)
    plot_face_pairs(embedding_model, pairs, pair_labels, X_test, y_test, target_names)
    tf.keras.backend.clear_session()
